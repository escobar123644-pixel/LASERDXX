...
export const Viewer: React.FC<ViewerProps> = ({ polylines, labels = [], onPolylinesChange }) => {
  ...
  const toggleLayer = (id: string) => {
    const updated = polylines.map(p => p.id === id ? { ...p, layer: p.layer === 'CUT' ? 'BOARDS' : 'CUT' } : p);
  ...
  return (
    <div className="relative w-full h-full bg-slate-950 overflow-hidden select-none">
      <div className="absolute top-4 right-4 flex flex-col gap-2 z-10">
        ...
        <button onClick={fitScreen} className="p-2 bg-slate-800 text-emerald-400 rounded hover:bg-slate-700 border border-slate-700 shadow"><Maximize size={20} /></button>
      </div>
          ...
          <g transform={`translate(${offset.x}, ${offset.y}) scale(${scale}, ${-scale})`}>
            {polylines.map((poly) => (
              <path key={poly.id}
                ...
                strokeWidth={1.5 / scale} vectorEffect="non-scaling-stroke"
            ...
            {/* ETIQUETAS VISIBLES Y DERECHAS */}
            {labels && labels.map((lbl, idx) => (
              <text key={`lbl-${idx}`} x={lbl.x} y={lbl.y}
                fill="#ef4444" fontSize={lbl.height}
                textAnchor="middle" alignmentBaseline="middle"
                // Transformación clave: Escala local 1,-1 para des-invertir y trasladar
                transform={`translate(${lbl.x}, ${lbl.y}) scale(1, -1) translate(${-lbl.x}, ${-lbl.y})`}
                style={{ pointerEvents: 'none', userSelect: 'none', textShadow: '0px 0px 5px black', fontWeight: 'bold' }}>
                {lbl.text}
              </text>
            ))}
          </g>
