    ...
    <div className="h-screen flex flex-col bg-slate-950 text-slate-300 overflow-hidden font-sans">
      ...
      <header className="h-16 bg-slate-900 border-b border-slate-800 flex items-center justify-between px-6 shrink-0 z-30 relative shadow-md">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="font-bold text-lg tracking-wider text-slate-100 italic">DXF PRO <span className="text-emerald-500">//</span> ALBERPROYEC</h1>
            <div className="flex items-center gap-2 text-[10px] font-mono text-slate-500">
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
              OP: {session.username} [{session.role}]
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          {session.role === 'ADMIN' && (
            <button onClick={() => setIsAdminOpen(!isAdminOpen)} className={`p-2 transition-colors ${isAdminOpen ? 'text-emerald-500' : 'text-slate-400 hover:text-emerald-400'}`} title="Admin Settings">
              <Settings size={20} />
            </button>
          )}
                  ...
                  <div className="space-y-4 font-mono text-sm">
                     ...
                     {showMetrics && (
                       <div className="mt-6 pt-4 border-t border-slate-800 space-y-3">
                         <div>
                           <div className="text-xs text-slate-500 mb-1 uppercase">Consumption (Y)</div>
              ...
              <div className="p-6 flex-1 overflow-y-auto">
                 ...
                 <div className="space-y-3 font-mono text-xs">
                    <div className="flex items-center gap-3">
                       ...
                       <span className="text-emerald-500 font-bold">CUT // CONTORNO</span>
                    </div>
                    <div className="flex items-center gap-3">
                       <div className="w-4 h-4 border border-red-500 bg-red-500/10"></div>
                       <span className="text-red-500 font-bold">BOARDS // HUECOS</span>
                    </div>
                 </div>
              </div>
            </aside>
               ...
               {polylines.length > 0 ? (
                 ...
                 <div className="absolute inset-0 flex items-center justify-center text-slate-700">
                    <div className="text-center">
                        <Upload size={48} className="mx-auto mb-4 opacity-50 text-emerald-500" />
                        <p className="text-lg font-medium text-slate-400">SISTEMA LISTO</p>
                        <p className="text-xs uppercase tracking-widest text-slate-600">Cargar DXF para procesar</p>
                    </div>
                 </div>
               )}
            </main>
      ...
      <footer className="h-8 bg-slate-900 border-t border-slate-800 flex items-center px-6 justify-between text-[10px] text-slate-600 font-mono">
        <span>ALBERPROYEC CLOUD v2.1.0</span>
        <span className="text-emerald-900/50">SECURE_SERVER_CONNECTED</span>
      </footer>
    </div>
  );
}
